...
public class BannerColorSwitchScreen extends Screen {
    ...
    private static final int PANEL_W =
            PAD + LAYOUT_COLS * SLOT_W + (LAYOUT_COLS - 1) * COL_GAP + PAD;
    ...
    protected void init() {
        ...
        for (int slotIdx = sourceColors.size(); slotIdx < MAX_SLOTS; slotIdx++) {
            ...
            int btnX = slotX(px, col) + ICON_SIZE + 6 + ARROW_W + 6;
            int btnY = slotY(py, row) + (ROW_H - TARGET_BTN_H) / 2;
            Button btn = this.addRenderableWidget(new Button.Plain(btnX, btnY, TARGET_BTN_W, TARGET_BTN_H,
                    Component.empty(), button -> {}, s -> s.get()) {
                @Override
                public void extractContents(GuiGraphicsExtractor g, int mx, int my, float a) {
                    this.extractDefaultSprite(g);
                    g.blit(RenderPipelines.GUI_TEXTURED, DYE_OUTLINE_ICON,
                            this.getX() + 2, this.getY() + 2, 0f, 0f, 16, 16, 16, 16);
                }
            });
            btn.active = false;
        }
        ...
        this.addRenderableWidget(Button.builder(
                Component.translatable("loom-assistant.common.ok"), button -> applyAndClose())
                .bounds(okCancelX, okY, SLOT_W, 20)
                .build());

        this.addRenderableWidget(Button.builder(
                Component.translatable("loom-assistant.common.cancel"), button -> this.onClose())
            ...
            if (active) {
                ...
                if (isIn(mouseX, mouseY, sx, iconY, ICON_SIZE, ICON_SIZE)) {
                    ctx.setTooltipForNextFrame(this.font,
                            List.of(dyeStack(source).getHoverName()), Optional.empty(), mouseX, mouseY);
                }
            } else {
                // Inactive source icon placeholder
                int iconY = centerY - ICON_SIZE / 2;
                ctx.blit(RenderPipelines.GUI_TEXTURED, DYE_OUTLINE_ICON,
                        sx, iconY, 0f, 0f, 16, 16, 16, 16);
                ...
                ctx.blit(RenderPipelines.GUI_TEXTURED, REPLACE_COLOR_ICON,
                        arrowX, centerY - ARROW_H / 2, 0f, 0f, ARROW_W, ARROW_H, ARROW_W, ARROW_H);
    ...
    private void drawPicker(GuiGraphicsExtractor ctx, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < pickerColors.length; i++) {
            ...
            if (hover) {
                ctx.setTooltipForNextFrame(this.font,
                        List.of(dyeStack(color).getHoverName()), Optional.empty(), mouseX, mouseY);
            }
        }
    }
